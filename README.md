...
[DVD et Blu-ray.json](Workflow/DVD%20et%20Blu-ray.json) contient 20 nœuds.

Extraction de **Clear a data table4** à **Merge11** : contrôle du cookie, récupération de la collection, insertion dans la table DVD, séparation films/séries, supports, souhaits et prix, puis Google Sheets. Les prix absents sont comptabilisés séparément des montants connus.

## Configuration du service

Renseigner le cookie dans **Edit Fields1**, remplacer toutes les occurrences de `YOUR_BLURAY_USER_ID` dans les contextes et le script HTTP, puis configurer Browserless et Discord. Le workflow commence par vider sa table DVD : utiliser une table dédiée et vérifier la session avant la première exécution.
